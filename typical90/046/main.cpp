...
struct Solver {
  void Solve() {
    ...
    ll ans = 0;
    for(auto it1 = mpa.be(); it1 != mpa.en(); ++it1) {
        for(auto it2 = mpb.be(); it2 != mpb.en(); ++it2) {
            for(auto it3 = mpc.be(); it3 != mpc.en(); ++it3) {
                if((it1->fi+it2->fi+it3->fi)%46 == 0) ans += (it1->se)*(it2->se)*(it3->se);
            }
        }
    }
    cout << ans << endl;
