...
bool f(int len) {//長さlen以上になるように分けるピースの数はk個以上ですか?
  int left_idx = 0;
  int right_idx = 0;
  srep(i,0,k) {
    while(a[right_idx]-a[left_idx] < len && right_idx < sz(a)) right_idx++;
    if(right_idx == sz(a)) return false;
    left_idx = right_idx;
  }
  return true;
}
