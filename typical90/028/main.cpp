...
struct Solver {
  void Solve() {
    int n;
    cin >> n;
    vector<vector<int>> G(1100,vector<int>(1100));
    rep(i,0,n) {
      int lx,ly,rx,ry;
      cin >> lx >> ly >> rx >> ry;
      G[lx][ly]++;
    ...
    rep(i,0,1099) rep(j,0,1099) G[i+1][j] += G[i][j];
    rep(i,0,1099) rep(j,0,1099) G[i][j+1] += G[i][j];
    vl ans(1100);
    srep(i,0,1000) srep(j,0,1000) ans[G[i][j]]++;
