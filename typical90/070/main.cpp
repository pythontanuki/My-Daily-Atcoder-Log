...
ll calc(vl values) {
    ...
    ll mid = values[si/2];
    ll res = 0;
    rep(i,0,si) res += abs(values[i]-mid);
    return res;
}

struct Solver {
  void Solve() {
    //マンハッタン距離はx、yそれぞれを独立に考えることができることに注意
    //min(sum(xs))+min(sum(ys))を求めるときに、発電所の場所はvalues[floor(len(values)/2)]とするのが最適であることがわかる
    int n;
    cin >> n;
    vl xs, ys;
    rep(i,0,n) {
