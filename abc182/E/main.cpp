...
struct Solver {
  void Solve() {
    ...
    rep(i,0,h) rep(j,0,w) tmp[i][j] = field[i][j];
    rep(i,0,h) rep(j,0,w) {
        if(tmp[i][j] == 2) continue;
        if(j-1 >= 0 && tmp[i][j-1] == 1) tmp[i][j] = 1;
    }
    rep(i,0,h)rep(j,0,w) if(tmp[i][j] == 1) ans[i][j] = 1;

    rep(i,0,h) rep(j,0,w) tmp[i][j] = field[i][j];
    rep(i,0,h) drep(j,w-1,0) {
        if(tmp[i][j] == 2) continue;
        if(j+1 < w && tmp[i][j+1] == 1) tmp[i][j] = 1;
    }
    rep(i,0,h)rep(j,0,w) if(tmp[i][j] == 1) ans[i][j] = 1;
    ...
    rep(j,0,w) rep(i,0,h) {
        if(tmp[i][j] == 2) continue;
        if(i-1 >= 0 && tmp[i-1][j] == 1) tmp[i][j] = 1;
    }
    rep(i,0,h)rep(j,0,w) if(tmp[i][j] == 1) ans[i][j] = 1;


    rep(i,0,h) rep(j,0,w) tmp[i][j] = field[i][j];
    rep(j,0,w) drep(i,h-1,0) {
        if(tmp[i][j] == 2) continue;
        if(i+1 < h && tmp[i+1][j] == 1) tmp[i][j] = 1;
    }
    rep(i,0,h)rep(j,0,w) if(tmp[i][j] == 1) ans[i][j] = 1;
