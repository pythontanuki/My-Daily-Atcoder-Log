...
struct Solver {
  void Solve() {
    //あるラインより上側は全部取って、ライン上は中途半端、ラインより下は全部取らない
    //すなわちそのラインがわかれば最終系がわかる。　二分探索
    ll n, k;
    CIN(n,k);
    vl a(n);
    rep(i,0,n) CIN(a[i]);
    ...
    ll top = 2e9, bottom = 0; //top: ok bottom: ng

    auto ok = [&](ll line) -> bool {
      ll cnt = 0;
      for(auto x : a) if(x >= line) cnt += x - line;
      return cnt <= k;
    };
