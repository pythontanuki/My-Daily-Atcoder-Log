...
string fill_zero(string s) {
    int si = sz(s);
    if (si == 1) rep(i,0,3) s = '0'+s;
    if (si == 2) rep(i,0,2) s = '0'+s;
    if (si == 3) rep(i,0,1) s = '0'+s;
    return s;
}
...
struct Solver {
  void Solve() {
    ...
    rep(num,0,10000) {
        ...
        t = fill_zero(t);
        vb check(10,false);
        rep(i,0,4) check[t[i]-'0'] = true;
        rep(i,0,10) {
            if(s[i] == 'o' && !check[i]) {ans--; break;}
            else if(s[i] == 'x' && check[i]) {ans--; break;}
        }
    }
    cout << ans << endl;
  }
