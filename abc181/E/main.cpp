...
struct Solver {
  void Solve() {
    ...
    right[0] = 0;
    for(int i = 2; i < n; i += 2) {
      left[i] = left[i-2] + llabs(h[i-1]-h[i-2]);
      right[i] = right[i-2] + llabs(h[n-i+1] - h[n-i]);
    }

    ll ans = LINF;
    for(auto x : w) {
      ll pre = 0;
      int ind = lower_bound(ALL(h),x) - h.begin();
      if(ind % 2) pre += left[ind-1]+right[n-ind]+(x-h[ind-1]);
      else pre += left[ind] + right[n-ind-1] + h[ind]-x;
      chmin(ans,pre);
    }
    cout << ans << endl;
  }
