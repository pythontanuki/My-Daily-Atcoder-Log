...
struct Solver {
  void Solve() {
    //解説AC
    //マンハッタン距離は、45度回転するとX+Y同士の差の最大値とX-Y同士の差の最大値のmaxになる
    int n;
    CIN(n);
    vl x(n), y(n);
    rep(i,0,n) CIN(x[i], y[i]);
    vector<ll> x_plus_y(n);
    vector<ll> x_minus_y(n);
    ...
    ll x_plus_y_max = -LINF;
    ll x_plus_y_min = LINF;
    ll x_minus_y_max = -LINF;
    ll x_minus_y_min = LINF;

    rep(i,0,n) chmax(x_plus_y_max, x_plus_y[i]);
    rep(i,0,n) chmin(x_plus_y_min, x_plus_y[i]);
    rep(i,0,n) chmax(x_minus_y_max, x_minus_y[i]);
    rep(i,0,n) chmin(x_minus_y_min, x_minus_y[i]);
    
    COUT(max(x_plus_y_max - x_plus_y_min, x_minus_y_max - x_minus_y_min));
  }
};
