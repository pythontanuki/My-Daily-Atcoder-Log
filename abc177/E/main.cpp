...
struct Solver {
  void Solve() {
    ...
    //pairwise coprime どの2個を持ってきても互いに素
    //setwisecoprime pairwisecoprimeではないが全体のgcdが互いに素 -->　素因数配列を作ってやれば良い
    //どちらでもないならnot coprime --> これは自明にもとまる
    //0とxのgcdはxになることを利用すればうまく実装できる
    bool pairwise = true;
    for(int i = 2; i <= M; i++) {
        int cnt = 0;
        for(int j = i; j <= M; j += i) {
    ...
    if(pairwise) ret("pairwise coprime");
    int g = 0;
    rep(i,0,n) g = gcd(g,a[i]);
    if(g == 1) ret("setwise coprime");
    ret("not coprime");
  }
};
