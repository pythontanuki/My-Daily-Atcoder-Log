...
struct Solver {
  void Solve() {
    ...
    rep(i,0,n) CIN(wv[i].fi, wv[i].se);
    S(rALL(wv));
    vector<int> x(m);
    rep(i,0,m) CIN(x[i]);
    while(tt--) { //容量の大きい荷物から見ていって、入れられる箱があるならすぐに入れる
        int l, r;
        CIN(l,r);
        --l;--r;
        vector<bool> check(m,true);
        vector<bool> used(n,true);
        srep(i,l,r) check[i] = false;
        vector<pair<int,int>> ok_box;
        rep(i,0,m) if(check[i]) ok_box.push_back({x[i],i});
        S(ALL(ok_box));
        int ans = 0;
        for(auto [max_weight, box_index] : ok_box) {
            vector<pair<int,int>> eachok;
            rep(ni,0,n) {
                int weight = wv[ni].fi;
                int value = wv[ni].se;
                if(!check[box_index]) continue;
                if(!used[ni]) continue;
                if(max_weight < weight) continue;
                eachok.push_back({value,ni});
            }
            if(!sz(eachok)) continue;
            S(rALL(eachok));   
            ans += eachok[0].fi;
            check[box_index] = false;
            used[eachok[0].se] = false;
        }
        cout << ans << endl;
    }
  }
