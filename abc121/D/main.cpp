...
struct Solver {
  void Solve() {
    ...
    for(ll i = 1; i <= (1LL << mx); i *= 2) {
        ll na = a / (i*2);
        ll ma = a % (i*2);
        ll nna = max(na * i + max(ma-i+1,0LL),0LL);
        ll nb = b / (i*2);
        ll mb = b % (i*2);
        ll nnb = max(nb * i + max(0LL,(mb-i+1)),0LL);
        int dnna = nna % 2;
        int dnnb = nnb % 2;
        char c = abs(dnna - dnnb) + '0';
        ans += c;
    }
    int n = sz(ans);
    ll tot = 0;
    rep(keta,0,n) tot += binary_pow(2,keta) * (ans[keta]-'0');
    COUT(tot);
  }
};
