...
int main() {
    ...
    cin.tie(nullptr);
    //重みでソートしておくことで、辺を追加した時に、ある点からある点へ移動する際に、追加した点が必ずコスト最大の辺になる
    //この時、追加した辺を境に２つのグループができる。
    //よって、グループの中からそれぞれ１つずつ選んだパスが題意のパスであり、これにコストをかけてやれば総和がもとまる。
    int n;
    read(n);
    vector<pair<int,P>> to(n);
    rep(i,0,n-1) {
        int u,v,w;
        read(u,v,w);
        --u; --v;
        to.push_back({w,P(u,v)});
    }
    dsu d(n);
    S(ALL(to));
    ll ans = 0;
