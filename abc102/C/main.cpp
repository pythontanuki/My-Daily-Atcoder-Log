...
struct Solver {
  void Solve() {
    ...
    //これは、変形すると abs((A1-1)-b) + abs((A2-2)-b) + abs((A3-3)-b) + ... abs((An-n)-b)
    //Ai-iをBiとすると、abs(B1-b)+abs(B2-b)+abs(B3-b)+...abs(Bn-b)とできる。
    //これはマンハッタン距離の総和そのままである。(wikipedia参照)
    //すなわち、Ai-iの数列の中央値からの距離の合計値が答えになる。
    cin >> n;
    a.resize(n);
    b.resize(n);
    rep(i,0,n) {
