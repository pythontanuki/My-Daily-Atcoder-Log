...
struct Solver {
  void Solve() {
    //解説AC
    //Greedyにやるのは分かっていたが実装が難しい
    //前から見ていくときに、単調増加のフラグと単調減少のフラグが同時に立っていたら区切る
    //区切られた数列の個数は、植木算の考え方でインクリメントすれば答え
    int n;
    cin >> n;
    vl a(n);
    rep(i,0,n) cin >> a[i];
    ...
    bool down_fl = false;
    rep(i,0,n-1) {
        if(a[i] < a[i+1]) up_fl = true;
        else if(a[i] > a[i+1]) down_fl = true;
        if(up_fl && down_fl) {
            ans++;
            up_fl = false;
            down_fl = false;
