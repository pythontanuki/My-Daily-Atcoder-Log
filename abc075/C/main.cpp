...
struct Solver {
  void Solve() {
    //各辺ごとにtoから辺eを除いたGを作成し、それが連結かどうかを判定する
    int n, m;
    cin >> n >> m;
    vi x(m), y(m);
    rep(i,0,m) {
    ...
    int ans = 0;
    //各辺を1つずつ取り除いていき、そのグラフが非連結かどうかを判定する　非連結ならその辺は橋である
    rep(e,0,m) {
        dsu d(n);
        rep(i,0,m) {
            if(e == i) continue;
        ...
        int cnt = 0;
        rep(i,0,n) if(i == d.leader(i)) cnt++; //そのグループのリーダーが自分自身自分自身と一致するのは、連結成分なら1つだけ
        if(cnt > 1) ans++;//非連結なら答えをインクリメント
    }
    cout << ans << endl;
  }
};
