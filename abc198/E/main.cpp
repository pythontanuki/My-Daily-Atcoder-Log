...
void dfs(int v, int p = -1) {
    if(cnt[c[v]] == 0) ok[v] = true;//ここでvに到達するまでの最短経路における色の分布がわかる
    cnt[c[v]]++;
    for(auto u : to[v]) {
        if(u == p) continue;
        dfs(u,v);
    }
    cnt[c[v]]--;//帰りがけ時に頂点uの色のカウントを1減らす　//行くところがなくなったらここを通る //continueはループ内処理をスキップするだけである
}
...
//         ans.pb(v);
//     }
//     ans.pb(v+10);//行くところがなくなったらここを通る //continueはループ内処理をスキップするだけである
// }
...
//     }
//     dfs(0);
//     rep(i,0,sz(ans)) cout << ans[i]+1 << endl;
//   }
// };
