...
struct Solver {
  void Solve() {
    ...
    CIN(n,x);
    vpl ab(n);
    rep(i,0,n) CIN(ab[i].fi, ab[i].se);
    vector<ll> s(n);
    s[0] = ab[0].fi + ab[0].se;
    rep(i,0,n-1) s[i+1] = s[i] + ab[i+1].fi + ab[i+1].se;
    //第nステージを遊ぶには第n-1ステージまでは全てゲームとビデオを両方する必要がある。これは累積和出前計算できる
    //第nステージまでで、最も早くクリアできるゲームだけをプレイし続けるのが最適
    vector<ll> fastest(n,LINF);//第nステージまでで一番早くクリアできるゲームの時間
    fastest[0] = ab[0].se;
    rep(i,1,n) chmin(fastest[i],min(fastest[i-1],ab[i].se));
    ll ans = LINF;
    rep(i,0,n) {
        ll px = x;
        ll pre = 0;
