...
struct Solver {
  void Solve() {
    //素因数分解の一意生からPの素因数分解はaの各因数の素因数分解の積と一致する
    //素因数の最大公約数をgとするとg^n|Pという関係が成り立つ
    //このときP = x^e1 * x^e2 * x^e3 * ... x^ekと素因数分解できる
    //また、gも素因数分解するとg = x^f1 * x^f2 * x^f3 * ... x^fkと素因数分解できる (必ずf < e)
    //このときfi*n <= eiが成り立ち、このとき最大のfiは[ei/n]となる
